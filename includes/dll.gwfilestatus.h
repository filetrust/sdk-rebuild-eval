...
*   For the @ref gwFileProcessingFileToFile functions the negative values are utilised to report issues with file output (possibliy in addition to file processing issues)
...
typedef enum
{
    ...
    /* This value indicates that the document was exported successfully, but failures occured when writing the archive package. (Export mode) */
    eGwFileStatus_SuccessExportWriteFailure = -12,

    /* This value indicates that the document was non-conformant in some way, and failures occured when writing the archive package. (Export mode) s*/
